...
typedef struct {
    ...
    int num_ids;         /* number of alloc/realloc ids 对应的是short1-bal.rep文件里面第二行操作次数*/
    int num_ops;         /* number of distinct requests 对应的是short1-bal.rep文件里面第三行总的操作次数*/
    int weight;          /* weight for this trace (unused) */
    traceop_t *ops;      /* array of requests 主要目的是记录当前request类型和索引以及块大小(未对齐)*/
    char **blocks;       /* array of ptrs returned by malloc/realloc... 指向内存块的指针*/
...
int main(int argc, char **argv)
{
    ...
    trace_t *trace = NULL;     /* stores a single trace file in memory 具体注释在对应的结构体里面已经声明*/
    range_t *ranges = NULL;    /* keeps track of block extents for one trace 单链表记录当前请求快的起始和结束地址以及指向下个请求的指针*/
	...
	case 'g': /* Generate summary info for the autograder */
	    ...
	    strcpy(tracedir, "./"); 
            tracefiles[0] = strdup(optarg);//optarg 指向参数 -f 对应的值，系统维护的一个全局变量 -V -f short1-bal.rep, 该例子指向short1-bal.rep这个参数
            tracefiles[1] = NULL;
            break;
     ...
     */
    if (run_libc) { //走系统的malloc，不是本实验的重点。忽略
	if (verbose > 1)
	    printf("\nTesting libc malloc\n");
    ...
    mem_init(); //初始化一个20M大小的堆

    /* Evaluate student's mm malloc package using the K-best scheme */
    for (i=0; i < num_tracefiles; i++) {
	trace = read_trace(tracedir, tracefiles[i]);//记录保存short1-bal.rep的内容。后面要用，read_trace逻辑仔细看下很简单，不赘述了。
	mm_stats[i].ops = trace->num_ops;
	if (verbose > 1)
	    printf("Checking mm_malloc for correctness, ");
	mm_stats[i].valid = eval_mm_valid(trace, i, &ranges); //评估我们实现的mm.c 的效率主要函数，重点关注
	if (mm_stats[i].valid) {
	    if (verbose > 1)
		printf("efficiency, ");
		     ...
		     int tracenum, int opnum)
{
	...
	* 这个注释开始理解成地址p需要对齐，但是 IS_ALIGNED宏实现的是p指向的内容要字节对齐。
	* 实际操作下来看 按照(*)(unsigned int *)(lo) 取出来的内容始终不对。检查了代码，否定了实现问题。
	* 经过排查发现是 memset(p, index & 0xFF, size); 设置完，free 时候没清除导致 下次在用这块内存的时候IS_ALIGNED检查始终过不了。
	* 所以在free阶段函数 添加了一段代码 memset(lo, 0, size); 测试能过。但是这个和系统free之后的不会处理对应的内存的数据有些出入，暂时不确定这样做是否妥当。
	*/
    if (!IS_ALIGNED(lo)) {
...
static int eval_mm_valid(trace_t *trace, int tracenum, range_t **ranges) 
{
	...
	int ret = mm_init(); //初始化mm_init，内部注释很详细。
    if (ret < 0) {
    ...
    for (i = 0;  i < trace->num_ops;  i++) { //处理前面解析的short1-bal.rep内容，分为 malloc、realloc和 free
	...
	size = trace->ops[i].size;
	    ...
	    if ((p = mm_malloc(size)) == NULL) { //主要函数逻辑，申请size大小内存，8字节对齐。
	     ...
	     */ 
	    if (add_range(ranges, p, size, tracenum, i) == 0) //检查mm_malloc 分配的块的大小和内存对齐的正确性
			return 0;
